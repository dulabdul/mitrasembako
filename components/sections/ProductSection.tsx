...
import { Star, MessageCircle } from 'lucide-react'; // Import icon WA
import { products, Product } from '@/data/products';
import { SITE_CONFIG } from '@/data/content'; // Import config untuk nomor WA
      ...
      <Container>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6'>
          {products.map((product) => {
            // Logic pesan WA per produk
            const waMessage = `Halo Mitra Sembako, saya ingin pesan produk: ${product.name}. Apakah stok tersedia?`;
                ...
                <div className='p-5 flex-1 flex flex-col'>
                  ...
                  <div className='flex gap-1 mb-2'>
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className='w-4 h-4 text-[var(--color-secondary)] fill-current'
                      />
                    ))}
                  </div>
                  ...
                  <div className='mt-auto flex flex-col gap-3'>
                    {/* Tombol Lihat Harga (Modal) */}
                    <Button
                      className='w-full'
                      variant='outline'
                      onClick={() => setSelectedProduct(product)}
                      aria-label={`Lihat daftar harga ${product.name}`}>
                      Lihat Daftar Harga
                    </Button>

                    {/* Tombol WA Langsung (New) */}
                    <Button
                      className='w-full'
                      variant='whatsapp'
                      href={waLink}
                      ...
                      <span className='ml-1'>Pesan Sekarang</span>
                    </Button>
                  </div>
                </div>
              </div>
